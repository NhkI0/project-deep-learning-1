...
def load_and_preprocess_data():
    ...
    X = df.drop(
        target_columns, axis=1, errors="ignore"
    )  # On retire les columns cible du dataset
    y = df[target_columns]
...
def training():
    for epoch in range(EPOCHS):
        ...
        for batch_id, (batch_x, batch_y) in enumerate(train_loader):
            ...
            print(
                f"\rEpochs:{epoch + 1} Batch:{batch_id + 1} Loss:{loss.item():.4f} Time:{elapsed_time:.2f} sec",
                end="",
            )
